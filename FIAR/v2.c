...
void mode3(){
	int li,lj,i=1,count=0,test1=0,test2=0,test3=0,test4=0;
	while(strcmp(cquit,"quit")&&strcmp(cquit,"q")) //每局开局前输入一串字符，若为quit则退出程序
	{
  //   	srand(time(NULL));
  //   	li=rand()%15;
...
/////////////////////////////////////////////////////////////////////////////////
    	...
    	for(count=0;(aRecordBoard[li][lj]!=0||!isFour(aRecordBoard2,li,lj,(i+1)%2+1))&&count<=555000;count++){
    		li=rand()%15;
    		lj=rand()%15;
    	}
    	if(!isFour(aRecordBoard2,li,lj,(i+1)%2+1)){
    		for(count=0;(aRecordBoard[li][lj]!=0||!isThree(aRecordBoard2,li,lj,(i+1)%2))&&count<=555000;count++){
    		li=rand()%15;
    		lj=rand()%15;
    		}
    		// if(isThree(aRecordBoard2,li,lj,(i+1)%2)){
    		// for(count=0;(aRecordBoard[li][lj]!=0||!isThree(aRecordBoard2,li,lj,(i+1)%2+1))&&count<=555000;count++){
    		// li=rand()%15;
    		// lj=rand()%15;
    		// }
    		if(!isThree(aRecordBoard2,li,lj,(i+1)%2+1)){
    		for(count=0;(aRecordBoard[li][lj]!=0||!isTwo(aRecordBoard2,li,lj,(i+1)%2+1))&&count<=555000;count++){
    		li=rand()%15;
    		lj=rand()%15;
    		}
    		if(!isTwo(aRecordBoard2,li,lj,(i+1)%2+1)){
    			for(count=0;(aRecordBoard[li][lj]!=0||!isOne(aRecordBoard2,li,lj,(i+1)%2+1))&&count<=555000;count++){
    			li=rand()%15;
    			lj=rand()%15;
    			}
    		...
    		else if(isTwo(aRecordBoard2,li,lj,(i+1)%2+1))
    		test3=1;
    	}
    	else if(isThree(aRecordBoard2,li,lj,(i+1)%2+1))
    	test2=1;
		// }
    }

    else if(isFour(aRecordBoard2,li,lj,(i+1)%2+1))
    	test1=1;
	// else
 //     	while(aRecordBoard[li][lj]!=0||!isSurrounded(aRecordBoard2,li,lj,i)){
 //    	li=rand()%15;
