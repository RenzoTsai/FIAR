...
int li,lj,count=1,chosen_total=0;
...
///////////////////////////////////////////////////////
		...
		printf("%d%c\n",lci,lcj );
    	printf("%d\n",count);
    	printf("%d %d %d %d %d %d %d %d\n",chosen_total,board[li][lj].five,board[li][lj].a4,board[li][lj].four,board[li][lj].a3,board[li][lj].three,board[li][lj].a2,board[li][lj].two);
    	for(li=0;li<=14;li++)
			for(lj=0;lj<=14;lj++)
    			board[li][lj].total=board[li][lj].five=board[li][lj].a2=board[li][lj].four=board[li][lj].a4=board[li][lj].three=board[li][lj].a3=board[li][lj].two=chosen_total=0;
    	count++;
...
void mode4(){
	int i=0,j=0,chosen_total=0,count=1;
	while(strcmp(cquit,"quit")&&strcmp(cquit,"q")&&count<=225) //每局开局前输入一串字符，若为quit则退出程序
	{
		...
		for(li=0;li<=14;li++)
			for(lj=0;lj<=14;lj++){
				if(aRecordBoard2[li][lj]==0&&isFour(aRecordBoard2,li,lj,(count+1)%2+1))
					board[li][lj].five=1000;

				if(aRecordBoard2[li][lj]==0&&isFour(aRecordBoard2,li,lj,(count)%2+1))
					board[li][lj].a4=999;

				if(aRecordBoard2[li][lj]==0&&isThree(aRecordBoard2,li,lj,(count+1)%2+1)&&!isFour(aRecordBoard2,li,lj,(count+1)%2+1))
					board[li][lj].four=98;

				if(aRecordBoard2[li][lj]==0&&isThree(aRecordBoard2,li,lj,(count)%2+1))
					board[li][lj].a3=97;

				if(aRecordBoard2[li][lj]==0&&isTwo(aRecordBoard2,li,lj,(count+1)%2+1)&&!isThree(aRecordBoard2,li,lj,(count+1)%2+1)&&!isFour(aRecordBoard2,li,lj,(count+1)%2+1))
					board[li][lj].three=96;

				if(aRecordBoard2[li][lj]==0&&isTwo(aRecordBoard2,li,lj,count%2+1))
					board[li][lj].a2=95;

				if(aRecordBoard2[li][lj]==0&&isOne(aRecordBoard2,li,lj,(count+1)%2+1)&&!isTwo(aRecordBoard2,li,lj,(count+1)%2+1)&&!isThree(aRecordBoard2,li,lj,(count+1)%2+1)&&!isFour(aRecordBoard2,li,lj,(count+1)%2+1))
					board[li][lj].two=94;

				board[li][lj].total=board[li][lj].five+board[li][lj].a2+board[li][lj].four+board[li][lj].a4+board[li][lj].three+board[li][lj].a3+board[li][lj].two;
				if(board[li][lj].total){
				lcj=lj+'a';			
				lci=15-li;
				printf("%d%c\n",lci,lcj );
				printf("%d %d %d %d %d %d %d %d\n",board[li][lj].total,board[li][lj].five,board[li][lj].a4,board[li][lj].four,board[li][lj].a3,board[li][lj].three,board[li][lj].a2,board[li][lj].two);
				}
			}
...
void givingGrades(int s[SIZE][SIZE]){
	for(li=0;li<=14;li++)
			for(lj=0;lj<=14;lj++){
				...
				if(s[li][lj]==0&&isThree(s,li,lj,(count+1)%2+1)&&!isFour(s,li,lj,(count+1)%2+1))
					board[li][lj].four=200;

				if(s[li][lj]==0&&isThree(s,li,lj,(count)%2+1))
					board[li][lj].a3=191;
